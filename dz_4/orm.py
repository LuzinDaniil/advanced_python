...
class Field:
    ...
    def validate(self, value):
        ...
        if value is None:
            return value
        elif self.f_type == datetime:
            print(value)
            return str(value)
        elif self.f_type != type(value) and self.f_type != bool:
            print(value)
            print(type(value), self.f_type)
            raise TypeError
        try:
            self.f_type(value)
        except:
...
class DatetimeField(Field):
    def __init__(self, required=True, default=None):
        super().__init__(datetime, required, 0)
...
# User.objects.create(email='val@mail.ru', password='123', name='Иванова',
...
# user = User.objects.get(email='va10@mail.ru', password='qwert')
# print(user.id)
